...
def popularity_recommendation(input_song_name, num_recommendations=5):
    ...
    return most_popular_tracks.head(num_recommendations)
...
def knn_based_recommendations(input_song_name, num_recommendations=5):
    ...
    knn_based_recommendations = data.iloc[similar_song_indices][['track_name', 'artists', 'album_name', 'popularity', 'track_genre', 'duration_ms']]
    ...
    return knn_based_recommendations.sort_values(by='distance', ascending=True).head(num_recommendations)
...
def cluster_based_recommendations(input_song_name, num_recommendations=5):
    ...
    # Get the tracks in the same cluster
    ...
    return cluster_tracks.head(num_recommendations)
...
def hybrid_recommendation(input_song_name, num_recommendations=5):
    ...
    # filter by cluster
    candidate_songs = data[data['cluster'] == data.loc[data['track_name'] == input_song_name, 'cluster'].values[0]]

    # KNN model for similarity
    candidate_songs = knn_based_recommendations(input_song_name, num_recommendations=num_recommendations*3)
    ...
    # Return top N based on the hybrid score
    return candidate_songs.sort_values(by='hybrid_score', ascending=False).head(num_recommendations)
# Evaluation
input_song_name = 'Blinding Lights'
song = data.loc[data['track_name'] == input_song_name]
song = song[0:1]
song[['track_name', 'artists', 'popularity', 'track_genre', 'duration_ms']]
popularity_recommendation(input_song_name, num_recommendations=5)
knn_based_recommendations(input_song_name, num_recommendations=5)
cluster_based_recommendations(input_song_name, num_recommendations=5)
hybrid_recommendation(input_song_name, num_recommendations=5)
# Hasil dari rekomendasi cukup sesuai